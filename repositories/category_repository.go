...
func (repo *CategoryRepository) GetAll() ([]models.Category, error) {
	query := "SELECT id, name, description FROM tb_m_category"
...
func (repo *CategoryRepository) Create(category *models.Category) error {
	query := "INSERT INTO tb_m_category (name, description) VALUES ($1, $2) RETURNING id"
	err := repo.db.QueryRow(query, category.Name, category.Description).Scan(&category.ID)
	return err
}

func (repo *CategoryRepository) GetById(id int) (*models.Category, error) {
	query := "SELECT id, name, description FROM tb_m_category WHERE id = $1"
...
func (repo *CategoryRepository) Update(category *models.Category) error {
	query := "UPDATE tb_m_category SET name=$1, description=$2 WHERE id=$3"
...
func (repo *CategoryRepository) Delete(id int) error {
	query := "DELETE FROM tb_m_category WHERE id=$id"
